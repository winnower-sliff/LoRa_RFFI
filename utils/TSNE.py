import torch
import numpy as np
from sklearn.manifold import TSNE
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D


def tsne_3d_plot(input_features, input_labels, perplexity=30, max_iter=1000):

    """
    精简版 3D t-SNE 可视化函数

    :param input_features: 输入特征 (Tensor或NumPy数组), 形状 [N, 512]
    :param input_labels: 类别标签 (Tensor或NumPy数组), 形状 [N]
    :param perplexity: t-SNE 困惑度 (默认30)
    :param max_iter: 迭代次数 (默认1000)
    :return: matplotlib Figure对象
    """

    # 统一转换为 NumPy 格式
    input_features = input_features.numpy() if isinstance(input_features, torch.Tensor) else input_features
    input_labels = input_labels.numpy() if isinstance(input_labels, torch.Tensor) else input_labels

    # 执行 t-SNE 降维（使用 max_iter 代替弃用的 n_iter）
    tsne = TSNE(
        n_components=3, perplexity=perplexity, max_iter=max_iter, random_state=42
    )  # 关键修改点
    embed_3d = tsne.fit_transform(input_features)

    # 创建 Axes
    fig = plt.figure(figsize=(10, 8))
    plt.switch_backend("qtagg")  # 显式指定交互式后端
    ax = fig.add_subplot(111, projection="3d")

    # 绘制 3D 散点图
    scatter = ax.scatter(
        embed_3d[:, 0],
        embed_3d[:, 1],
        embed_3d[:, 2],
        c=input_labels,
        cmap="tab10",
        alpha=0.6,
        s=10,  # 点大小
    )

    # 添加颜色条（关联到Figure而非全局plt）
    cbar = fig.colorbar(scatter, ax=ax, pad=0.1)
    cbar.set_label("Class Label")

    # 设置坐标轴标签和标题（使用ax方法）
    ax.set_title("3D t-SNE Visualization", pad=15)
    ax.set_xlabel("X", labelpad=10)
    ax.set_ylabel("Y", labelpad=10)
    ax.set_zlabel("Z", labelpad=10)

    # 调整布局（操作Figure对象）
    fig.tight_layout()

    return fig  # 返回Figure对象，不自动显示


def tsne_2d_plot(input_features, input_labels, perplexity=30, max_iter=1000, title="2D t-SNE Visualization"):

    """
    2D t-SNE 可视化函数
    
    :param input_features: 输入特征 (Tensor或NumPy数组), 形状 [N, 512]
    :param input_labels: 类别标签 (Tensor或NumPy数组), 形状 [N]
    :param perplexity: t-SNE 困惑度 (默认30)
    :param max_iter: 最大迭代次数 (默认1000)
    :param title: 图表标题 (默认"2D t-SNE Visualization")
    :return: matplotlib Figure对象
    """

    # 统一转换为 NumPy 格式
    input_features = input_features.numpy() if isinstance(input_features, torch.Tensor) else input_features
    input_labels = input_labels.numpy() if isinstance(input_labels, torch.Tensor) else input_labels
    
    # 检查标签是否为单值
    unique_labels = np.unique(labels)
    n_colors = len(unique_labels)
    
    # 执行 t-SNE 降维到2D
    tsne = TSNE(
        n_components=2,  # 改为2维
        perplexity=perplexity,
        max_iter=max_iter,
        random_state=42
    )
    embed_2d = tsne.fit_transform(input_features)
    
    # 创建图形
    fig = plt.figure(figsize=(10, 8))
    plt.switch_backend("qtagg")  # 显式指定交互式后端
    ax = fig.add_subplot(111)
    
    # 使用不同的颜色和标记增强区分度
    colors = plt.cm.tab10(np.linspace(0, 1, n_colors)) if n_colors > 1 else ['blue']
    markers = ['o', 's', '^', 'v', '<', '>', 'p', '*', 'X', 'D', 'P', 'h', '+', 'x', '|', '_'] * 2
    
    # 绘制每个类别
    for i, cls in enumerate(unique_labels):
        # 选择当前类别的点和标签
        idx = input_labels == cls
        # 使用不同的颜色和标记样式
        ax.scatter(
            embed_2d[idx, 0],
            embed_2d[idx, 1],
            c=[colors[i]],
            marker=markers[i],
            alpha=0.7,
            s=50,
            edgecolors='w',
            linewidth=0.5,
            label=f'Class {cls}'
        )
    
    ax.set_title(title, fontsize=15, pad=15)
    ax.set_xlabel("TSNE Dimension 1", fontsize=12)
    ax.set_ylabel("TSNE Dimension 2", fontsize=12)
    ax.grid(True, linestyle='--', alpha=0.3)
    
    # 添加图例
    if len(unique_labels) > 1:
        ax.legend(loc='best', title='Classes', frameon=True, shadow=True)
    
    fig.tight_layout()
    return fig  # 返回Figure对象

# 示例用法
if __name__ == "__main__":

    n_classes = 5
    n_samples_per_class = 20
    n_features = 64

    # 生成随机数据作为示例
    features = torch.randn(n_classes * n_samples_per_class, n_features)
    labels = torch.cat(
        [torch.full((n_samples_per_class,), i) for i in range(n_classes)]
    )

    fig_3d = tsne_3d_plot(features, labels)
    fig_3d.savefig('./tmp_3d.png')
    plt.show()
    
    fig_2d = tsne_2d_plot(features, labels)
    fig_2d.savefig('./tmp_2d.png')
    plt.show()